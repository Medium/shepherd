...
function DependencyManager(graph) {
  ...
  this._nodeNames = {}
  this._deps = {}
  this._conditionalNodes = {}
}

/**
 * Check whether a node is conditional or not
 *
 * @param {string} nodeName
 * @return {boolean}
 */
DependencyManager.prototype.isConditional = function (nodeName) {
  return !!this._conditionalNodes[nodeName]
}
 ...
 * @param {boolean} isConditional whether this is a conditional node or not
 * @return {Object} an Object containing the node arg and nodeName
 *     as provided by utils.getNodeInfoFromBuild
 */
DependencyManager.prototype.addNode = function (node, isConditional) {
  if (isConditional) {
    this._conditionalNodes[node] = true
  }
