...
BuilderInstance.prototype._resolve = function (data, nodeName) {
  ...
  .then(function (deps) {
    ...
    if (this._config.handlers.timing && !isInternalNode) startTime = microtime.now()
  ...
  promise.then(function (result) {
    ...
    if (this._config.handlers.increment && !isInternalNode) this._config.handlers.increment('node.' + nodeName + '.success')
    if (this._config.handlers.timing && !isInternalNode) this._config.handlers.timing('node.' + nodeName, microtime.now() - startTime)
  }.bind(this))
  promise.fail(function (err) {
    if (traceInterval) clearInterval(traceInterval)
    this._trace('error', {traceId: data._traceId, node: nodeName, action: 'failed'})
    if (this._config.handlers.increment && !isInternalNode) this._config.handlers.increment('node.' + nodeName + '.failure')
    if (this._config.handlers.timing && !isInternalNode) this._config.handlers.timing('node.' + nodeName, microtime.now() - startTime)
  }.bind(this))
...
BuilderInstance.prototype.build = function (inputData, callback) {
  ...
  if (this._config.handlers.timing && this._outputs.length) startTime = microtime.now()
  ...
  .then(function (promises) {
    ...
    if (this._config.handlers.timing && this._outputs.length) this._config.handlers.timing('build.' + this._outputs ? this._outputs.join(',') : 'EMPTY', microtime.now() - startTime)
