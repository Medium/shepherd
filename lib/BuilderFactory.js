 ...
 * @param {Number} literalCounter optional counter to start literals at to prevent
...
BuilderFactory.prototype.provideTo = function (node, deps) {
  ...
  if (Array.isArray(deps)) this._nodes[node].deps = deps
    ...
    var found = false
    var nodeDeps = this._nodes[node].deps

    for (var key in deps) {
      var newDep = deps[key]

      if (key === '!') {
        if (!Array.isArray(newDep)) newDep = [newDep]

        // nodes this should run after
        for (var i = 0; i < newDep.length; i += 1) {
          var dep = (newDep[i].charAt(0) === '!') ? newDep[i] : '!' + newDep[i]
          if (nodeDeps.indexOf(dep) === -1) nodeDeps.push(dep)
        }
        ...
        var idx = nodeDeps.indexOf(key)
        if (idx !== -1) {
          // replace an existing node
          nodeDeps[idx] = newDep
        } else {
          throw new Error("Unknown node '" + key + "' provided to '" + node + "'")
        }
      }
    }
  } else {
