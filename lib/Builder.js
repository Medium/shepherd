...
Builder.prototype.builds = function (field) {
  if (arguments.length != 1) {
    throw new Error('.builds() must only have one argument')
  }

  if (typeof field === 'string') {
    var fieldObj = {}
    fieldObj[field] = utils.getNodeRealName(field)
  ...
  for (var alias in field) {
    ...
    if (utils.isPrivateNode(fieldName)) throw new Error('You may not build private nodes in this builder')

    if (utils.isSilentNode(fieldName)) throw new Error('! and ? operators must be on the key of the .builds() object and not the value')
...
/**
 * Add a modifier node to another node in this Builder
 *
 * @param {string} nodeName the node to modify
 * @param {string|Object} modifierData the modifier's node information
 */
Builder.prototype._addModifier = function (nodeName, modifierData) {
  var modifierInfo = utils.getNodeInfoFromModifier(this._graph, modifierData, nodeName)
  var modifierNodeName = modifierInfo.nodeName
  var nodeShortName = modifierInfo.arg

  // find a new name for the current node
  var i = 0
  var newName
  do {
    i++
    newName = utils.generateModifierNodeName(nodeName, i)
  } while (this._nodes[newName])

  // move the current node
  this._nodes[newName] = this._nodes[nodeName]
  this._nodes[newName].setAlias('!' + newName)

  // create a new node for the modifier
  var newNodeMap = {}
  newNodeMap[nodeName] = modifierNodeName
  var newNode
  if (this._nodes[modifierNodeName]) {
    newNode = this._nodes[nodeName] = this._nodes[modifierNodeName].clone()
    newNode.setAlias(newNode)
  } else {
    newNode = this.builds(newNodeMap)
  }

  // point the modifier at the old node
  var newNodeUsing = {}
  newNodeUsing[nodeShortName] = newName
  newNode.using(newNodeUsing)
}
...
Builder.prototype._resolve = function (data, nodeName) {
  ...
  if (!promises.length && node.isLiteral) {
    // shortcut if there are no inputs and this is a literal
    return data[nodeName] = data._hashedResponses[hashKey] = Q.resolve(node.literalValue)
  }

  // wait for dependencies to resolve
  data[nodeName] = data._hashedResponses[hashKey] = Q.all(promises)
    .then(function (results) {
      if (node.isLiteral) return node.literalValue
      if (node.isSubgraph) return results[node.argInputs.length - 1]

      // compose the args for the function to call
      var args = node.argInputs.length ? results.slice(0, node.argInputs.length) : []

      // this is mostly used for upcasting arguments into arrays
      if (node.isArrayWrapper) return args
    ...
    .fail(function (e) {
      if (!e.graphInfo) {
        var callerNodes = []
        for (var key in compiledNodes) {
          var i
          var found = false
          var compiledNode = compiledNodes[key]
        ...
        e.graphInfo = {
          ...
          callers: callerNodes,
          failureNodeChain: [],
          failureInputs: Object.keys(node.argInputs || {}),
          failureSilentInputs: (node.silentInputs || []).concat()
        }
      }
  ...
  if (config.enableProfiling) {
    data[nodeName]
      .fin(function () {
        if (startTime) self._profile(node.originalName, startTime)
      })
  }
