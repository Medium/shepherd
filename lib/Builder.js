...
Builder.prototype._generateHashesForNode = function (nodeName) {
  var node = this._compiled.nodes[nodeName]
  if (node.completeHash) return

  var completeHasher = crypto.createHash('md5')
  var nonSilentHasher = crypto.createHash('md5')

  // if the node doesn't exist, just hash the name
  if (!node.inputs) {
    completeHasher.update(nodeName)
    nonSilentHasher.update(nodeName)
    node.completeHash = completeHasher.digest('hex')
    node.nonSilentHash = nonSilentHasher.digest('hex')
    return
  }

  // loop through all inputs and use their hashes if they exist, otherwise hash their names
  for (var key in node.inputs) {
    var isSilent = node.silentInputs.indexOf(key) !== -1
    ...
    if (dep) this._generateHashesForNode(depNodeName)
    ...
    if (dep && dep.cacheDisabled) {
      ...
      completeHasher.update(dep.completeHash + depFullName.substr(depNodeName.length) + '|')
      nonSilentHasher.update(dep.nonSilentHash + depFullName.substr(depNodeName.length) + '|')
    } else {
      if (!dep) {
        completeHasher.update(depFullName + '|')
        if (!isSilent) nonSilentHasher.update(depFullName + '|')
      } else {
        completeHasher.update(dep.completeHash + depFullName.substr(depNodeName.length) + '|')
        if (!isSilent) nonSilentHasher.update(dep.nonSilentHash + depFullName.substr(depNodeName.length) + '|')
      }
    }
  }
  ...
  completeHasher.update(node.cacheDisabled ? node._hashIdx + '|' : '0|')
  nonSilentHasher.update(node.cacheDisabled ? node._hashIdx + '|' : '0|')

  node.completeHash = oid.hash(node.fn) + '-' + completeHasher.digest('hex')
  node.nonSilentHash = oid.hash(node.fn) + '-' + nonSilentHasher.digest('hex')
}
  ...
  for (i = 0; i < def.getBuilds().length; i += 1) {
    ...
    for (j = 0; j < toBuild.provides.length; j += 1) {
      ...
      if (provided.nodeName === 'args.*') {
        ...
        // node takes an argument that was passed into this node
        nodeName = provided.nodeName.substr(5)
        nodeRoot = utils.getNodeRootName(nodeName)
...
Builder.prototype._compileNode = function (originalNodeName, newNodeName, peers, nodeInputs, nodeModifiers, instance) {
  ...
  if (node.fn && node.fn._literal) {
    ...
    if (!literalPromises[literalHash]) literalPromises[literalHash] = Q.resolve(node.fn._literal)
 ...
 * @throws {Error}
 */
function _wrapError(builderName, nodes, node, e) {
  var key, found, i
  ...
  if (!e.graphInfo) {
    ...
    found = false
    for (i = 0; !found && i < depNodes.length; i++) {
...
function getSuccessHandler(node, nodeName, config, profiler) {
...
function getCompleteHandler(nodes, node, nodeName) {
  return function (response, data) {
    if (node.isOutput) {
...
function getSilentInputValidator(node) {
  if (!node.silentInputs.length) return noop
...
function getNonSilentInputValidator(node) {
  if (!node.argInputs.length) return noop

  return function (data, args) {
    var err, val

    // grab all arg inputs
    for (i = 0; i < node.argInputs.length && !err; i++) {
...
function getErrorHandler(builderName, nodes, node, nodeName) {
  var errorWrapper = _wrapError.bind(null, builderName, nodes, node)
  return function (e, data) {
    if (!(e instanceof ConditionalError)) errorWrapper(e)
...
Builder.prototype._getResolver = function (nodeName) {
  ...
  var onSuccess = getSuccessHandler(node, nodeName, config, profiler)
  var onError = getErrorHandler(this._name, nodes, node, nodeName)
  var onComplete = getCompleteHandler(nodes, node, nodeName)

  var hasSilentInputs = node.silentInputs.length > 0
  var validateSilentInputs = getSilentInputValidator(node)

  var hasNonSilentInputs = node.argInputs.length > 0
  var validateNonSilentInputs = getNonSilentInputValidator(node)
  ...
  if (node.literalPromise) {
    return this._resolvers[nodeName] = function (data) {
      ...
      if (hasSilentInputs) err = validateSilentInputs(data)
      if (hasNonSilentInputs && !err) err = validateNonSilentInputs(data, args)
    ...
    return this._resolvers[nodeName] = function (data) {
      ...
      if (!err && config.enableProfiling) data._startTimes[nodeName] = Date.now()
