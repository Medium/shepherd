...
Builder.prototype._compileNode = function (originalNodeName, newNodeName, peers, nodeInputs, nodeModifiers, instance) {
  ...
  var args = {}
  for (i = 0; i < def._args.length; i += 1) {
  ...
  for (i = 0; i < def._modifiers.length; i += 1) {
    modifier = utils.getNodeInfoFromModifier(this._graph, def._modifiers[i], originalNodeName)
  ...
  for (i = 0; i < def._builds.length; i += 1) {
    ...
    for (j = 0; j < toBuild.modifiers.length; j += 1) {
      modifier = utils.getNodeInfoFromModifier(this._graph, toBuild.modifiers[j], field)

      // build a new node (as a peer)
      var childRootNode = utils.getNodeRootName(modifier.nodeName)
      modifierMap[field].push(modifier)
      depManager.addDependency(field, childRootNode)
    }
    ...
    for (j = 0; j < toBuild.provides.length; j += 1) {
      ...
      if (provided.nodeName.length >= 5 && provided.nodeName.substr(0, 5) == 'args.') {
        ...
        // build a new node (as a peer)
        var childRootNode = utils.getNodeRootName(provided.nodeName)
  ...
  var children = this._compilePeers(depManager, provideMap, modifierMap, false)
  for (key in args) {
    var rootNode = utils.getNodeRootName(args[key])
    if (children[rootNode]) {
      inputs[key] = utils.swapNodeRoot(args[key], children[rootNode].newName)
    } else {
      inputs[key] = args[key]
    }
  }
  ...
  if (def._modifiers.length || nodeModifiers.length) {
    this._copyNode(newNodeName, newNodeName + '-modifier' + (++modifierIdx))
  }
  ...
  for (i = 0; i < def._modifiers.length; i += 1) {
    ...
    var modifier = modifierInfo[i]
    var currentNodeName = newNodeName + '-modifier' + modifierIdx
    var nextNodeName = newNodeName + '-modifier' + (++modifierIdx)
...
Builder.prototype._compilePeers = function (depManager, nodeInputs, nodeModifiers, isBuilder) {
  ...
  for (key in depManager._deps) depsToBuild[key] = true
  ...
  do {
    ...
    for (key in depsToBuild) {
      var nodeName = utils.getNodeRootName(depManager._nodeNames[key] || key)
      var deps = depManager._deps[key]

      compiledCurrentNode = true

      for (subkey in deps) {
        // if a peer dependency doesn't exist, mark the node as uncompilable
        if (!peers[subkey]) compiledCurrentNode = false
      }
...
Builder.prototype.compile = function () {
  ...
  for (key in this._nodes) {
    ...
    for (i = 0; i < node._modifiers.length; i += 1) {
      ...
      var childNodeRoot = utils.getNodeRootName(modifier.nodeName)
      depManager.addDependency(nodeRoot, childNodeRoot)
    }
    ...
    for (i = 0; i < node._inputs.length; i += 1) {
      ...
      var childNodeRoot = utils.getNodeRootName(input.nodeName)
      depManager.addDependency(nodeRoot, childNodeRoot)
    }
  }
  ...
  // create lists of required fields for each node
  var requiredFields = {}
  for (key in this._compiled.nodes) {
    var node = this._compiled.nodes[key]
    for (subkey in node.inputs) {
      var nodeParts = node.inputs[subkey].split('.')
      var nodeRoot = nodeParts.shift()
      if (this._compiled.nodes[nodeRoot] && this._compiled.nodes[nodeRoot].copyOf) nodeRoot = this._compiled.nodes[nodeRoot].copyOf
      if (!nodeParts.length || typeof requiredFields[nodeRoot] === 'string') requiredFields[nodeRoot] = '*'
      else {
        if (typeof requiredFields[nodeRoot] === 'undefined') requiredFields[nodeRoot] = {}
        requiredFields[nodeRoot][nodeParts.shift()] = 1
      }
    }
  }
  ...
  for (var key in this._silentBuilds) {
    var nodeParts = key.split('.')
    var nodeRoot = nodeParts.shift()
    if (this._compiled.nodes[nodeRoot] && this._compiled.nodes[nodeRoot].copyOf) nodeRoot = this._compiled.nodes[nodeRoot].copyOf
    if (!nodeParts.length || typeof requiredFields[nodeRoot] === 'string') requiredFields[nodeRoot] = '*'
...
Builder.prototype.resolve = function (data, nodeName) {
  ...
  if (data._hashedResponses[hashKey]) return data._hashedResponses[hashKey]
