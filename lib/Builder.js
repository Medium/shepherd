  ...
  if (runInputs) {
    ...
    while (nodesToCheck.length) {
      ...
      if (compiledNode && compiledNode.fn) {
        ...
        if (compiledNode && !compiledNode.fn) missingHandlers[nodeName] = true
        if (runInputs.indexOf(nodeName) === -1) missingNodes[nodeName] = true
      }
    }

    // show any missing handlers
    var missingHandlerKeys = Object.keys(missingHandlers)
    if (missingHandlerKeys.length) {
      throw new Error(missingHandlerKeys.map(function (nodeName) {
        return 'Node \'' + nodeName + '\' requires a handler function'
      }).join('. '))
    }
