...
Builder.prototype._getResolver = function (nodeName) {
  ...
  var resolver = this._resolvers[originalNodeName]
  ...
  if (nodeName.indexOf('.') !== -1) {
    ...
    return this._resolvers[originalNodeName] = this._createChildResolver.bind(this, getter)
  }

  if (typeof node === 'undefined') {
    // exit early if the node isn't found (it might be in the inputs)
    return this._resolvers[originalNodeName] = this._createInputResolver.bind(this, new Error("Unable to find node '" + nodeName + "'"), nodeName)
  }

  if (node && node.copyOf) {
    // copy of another node
    return this._resolvers[originalNodeName] = this._getResolver(node.copyOf)
  }
  ...
  return this._resolvers[originalNodeName] = getResolverFromHandlerChain(handlerChain)
}
