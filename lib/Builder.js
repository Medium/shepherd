...
Builder.prototype._validateBuilder = function (runInputs) {
  ...
  while (nodesToCheck.length) {
    ...
    if (compiledNode && compiledNode.fn) {
      ...
      if (this._config.enforceMatchingParams) {
        // TODO(kyle): Add support for shepherd key mappings
        var actualArgs = utils.parseFnParams(compiledNode.fn)
        var declaredArgs = compiledNode.argInputs
        var argsEqual = actualArgs.every(function(el, i) {
          return el === declaredArgs[i]
        })
        if (!argsEqual &&
            nodeName.indexOf(utils.NODE_PREFIX_BUILDER_OUTPUT) !== 0) {
          mismatchedParams[nodeName] = true
        }        
      }
    } else {
  ...
  if (this._config.enforceMatchingParams && mismatchedParamKeys.length) {
    throw new Error(mismatchedParamKeys.map(function(nodeName) {
      var node = this._compiled.nodes[nodeName]
      var err = 'Node "' + nodeName + '" has mismatched parameters: ' +
          'declared [' + node.argInputs.join(', ') + '] but were actually [' +
          utils.parseFnParams(node.fn).join(', ') + '].'
      return err
    }.bind(this)).join('\n'))
  }
}
