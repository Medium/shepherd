...
Builder.prototype.if = function (field) {
  return this._outputNode.if(field)
}

Builder.prototype.then = function () {
  return this._outputNode.then()
}

Builder.prototype.else = function () {
  return this._outputNode.else()
}
...
Builder.prototype.compile = function (runInputs) {
  ...
  for (key in this._compiled.nodes) {
    ...
    nodesToHashes[key] = node.completeHash
    if (!hashesToNodes[node.completeHash]) {
      hashesToNodes[node.completeHash] = key
    } else {
      this._compiled.nodes[key] = {
        copyOf: hashesToNodes[node.completeHash],
        completeHash: node.completeHash,
        nonSilentHash: node.nonSilentHash
      }
    }
  }
 ...
 */
Builder.prototype._generateHashForNode = function (nodeName, ignoreSilent) {
  var node = this._compiled.nodes[nodeName]
  ...
  for (var key in node.inputs) {
    if (!ignoreSilent || node.silentInputs.indexOf(key) === -1) {
      ...
      if (!this._compiled.nodes[depNodeName]) {
        ...
        hasher.update(this._compiled.nodes[depNodeName].completeHash + depFullName.substr(depNodeName.length) + '|')
      }
    }
  }
...
Builder.prototype._handleError = function (node, e) {
  ...
  for (var key in node.inputs) {
    ...
    console.log("INPUT", node.inputs[key])
  }
  ...
  if (!e.graphInfo) {
    ...
    for (var i = 0; !found && i < depNodes.length; i++) {
      ...
      console.log("Checking", depNode, lastFailureNode.nodeName)
...
Builder.prototype._createChildResolver = function (getter, context) {
  return context._resolve(getter.rootNode)
    .then(getter.childGetter)
}

Builder.prototype._createInputResolver = function (error, nodeName, context) {
  if (context[nodeName]) return context[nodeName]
  return context[nodeName] = Q.reject(error)
}
...
Builder.prototype._getResolver = function (nodeName) {
  ...
  if (nodeName.indexOf('.') !== -1) {
    ...
    return this._resolvers[nodeName] = this._createChildResolver.bind(this, getter)
  }

  // exit early if the node isn't found (it might be in the inputs)
  if (typeof node === 'undefined') {
    return this._resolvers[nodeName] = this._createInputResolver.bind(this, new Error("Unable to find node '" + nodeName + "'"), nodeName)
  }

  // copyOf
  if (node && node.copyOf) {
    return this._resolvers[nodeName] = this._getResolver(node.copyOf)
  }
  ...
  if (node.inputs && node.silentInputs.length) {
    handlerChain.push(this._getSilentDependencies.bind(this, node))
  }

  // build normal dependencies
  if (node.inputs && node.argInputs.length) {
    handlerChain.push(this._getVisibleDependencies.bind(this, node))
  }

  if (node.isLiteral) {
    // literal
    handlerChain.push(getLiteralFromNode(node))

  } else if (node.isSubgraph) {
    // subgraph
    handlerChain.push(getValueFromSubgraph)

  } else if (node.isArrayWrapper) {
    // array wrapper
    handlerChain.push(getArrayFromArguments)

  } else {

    if (this._config.useCallbacks) {
      // resolver with callback support
      handlerChain.push(function (data) {
        var context = data.context
        if (!node.cacheDisabled && context[nodeName]) return context[nodeName]
        if (!node.cacheDisabled && context._hashedResponses[node.nonSilentHash]) return context[nodeName] = context._hashedResponses[node.nonSilentHash]
        var args = data.args || []

        var deferred = Q.defer()
        args.push(deferred.makeNodeResolver())
        var result = node.fn.apply(null, args)
        return context[nodeName] = context._hashedResponses[node.nonSilentHash] = (typeof result === 'undefined' ? deferred.promise : result)
      })
      ...
      handlerChain.push(function (data) {
        var context = data.context
        if (!node.cacheDisabled && context[nodeName]) return data[nodeName]
        if (!node.cacheDisabled && context._hashedResponses[node.nonSilentHash]) return context[nodeName] = context._hashedResponses[node.nonSilentHash]
        var args = data.args || []

        if (self._config.enableProfiling) startTime = Date.now()
        context[nodeName] = context._hashedResponses[node.nonSilentHash] = node.fn.apply(null, args)
        if (self._config.enableProfiling) {
          Q.resolve(context[nodeName]).fin(function () {
            self._profile(node.originalName, startTime)
          })
        }
  ...
  return this._resolvers[nodeName] = getResolverFromHandlerChain(handlerChain)
}
  ...
  return function (context) {
    ...
    for (var i = 0; i < handlerChain.length; i++) {
      if (handlerChain[i].isError) {
        ...
        promise = promise.then(handlerChain[i])
      }
    }
    return promise
  }
