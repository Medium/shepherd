...
Builder.prototype._createChildResolver = function (getter, data) {
  return data._resolve(getter.rootNode)
    .then(getter.childGetter)
}

Builder.prototype._createErrorResolver = function (error, data) {
  return data[nodeName] = data._hashedResponses[completeHashKey] = Q.reject(error)
}
...
Builder.prototype._getSilentDependencies = function (node, data) {
  ...
  for (var i = 0; i < node.silentInputs.length; i += 1) {
    ...
    var promise = data._resolve(silentNodeName)
...
Builder.prototype._getVisibleDependencies = function (node, data) {
  ...
  for (var i = 0; i < node.argInputs.length; i += 1) {
    ...
    if (node.inputs[key] === '_requiredFields') {
      ...
      promises.push(data._resolve(node.inputs[key]))
    }
  }
  ...
  return Q.all(promises)
    .then(function (var_args) {
      return [data, Array.prototype.slice.call(arguments)]
    })
}
...
function getValueFromSubgraph(var_args) {
  return arguments[arguments.length - 1]
}

function getArrayFromArguments(arguments) {
...
Builder.prototype._getResolver = function (nodeName) {
  ...
  var compiledNodes = this._compiled.nodes
  var node = compiledNodes[nodeName]
  ...
  // exit early if the node isn't found
  if (typeof node === 'undefined') {
    return this._resolvers[nodeName] = this._createErrorResolver.bind(this, new Error("Unable to find node '" + nodeName + "'"))
  }

  // reject if the node if it's not found
  if (typeof node === 'undefined') {
    return this._resolvers[nodeName] = this._createErrorResolver.bind(this, new Error("Unable to find node '" + nodeName + "'"))
  }
  ...
  if (!node.isLiteral && !handlerChain.length) {
  ...
  if (node.isLiteral) {
    ...
    if (this._config.useCallbacks) {
      ...
      handlerChain.push(function (response) {
        var data = response[0]
        if (data._hashedResponses[node.nonSilentHash]) return data._hashedResponses[node.nonSilentHash]
        var args = response[1]
      ...
      handlerChain.push(function (response) {
        ...
        return data[nodeName] = data._hashedResponses[node.nonSilentHash] = node.fn.apply(null, args)
      })
    }
