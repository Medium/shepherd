...
Getter.prototype.hasError = function () {
  return !!this._err
}

Getter.prototype.setValue = function (val) {
  this._val = val
  return this
}

Getter.prototype.setError = function (err) {
  this._err = err
  return this
}
...
Builder.prototype._resolve = function (data, nodeName) {
  ...
  // helpers for tracking inputs and dependent nodes
  var depNodes = []
  var inputs = node.inputs

  // set up silent dependencies
  var depsPromise
  if (node.inputs && node.silentInputs.length) {
    ...
    for (i = 0; i < node.silentInputs.length; i += 1) {
      ...
      var silentNodeName = inputs[key]
      var promise = data._resolve(silentNodeName)
      if (!node.ignoredErrors[silentNodeName]) {
        promises.push(promise)
      }

      depNodes.push(silentNodeName)
    }
    depsPromise = Q.all(promises)
  } else {
    depsPromise = EMPTY_ARRAY_PROMISE
  }
  ...
  if (!depNodes.length && !node.argInputs.length && node.isLiteral) {
  ...
  if (node.inputs && node.argInputs.length) {
    for (i = 0; i < node.argInputs.length; i += 1) {
      depNodes.push(inputs[node.argInputs[i]])
    }

    // if there are non-silent inputs, start resolving them
  ...
  } else if (depsPromise !== EMPTY_ARRAY_PROMISE) {
    // there are no non-silent inputs, return
    depsPromise = depsPromise.then(returnEmptyArray)
  }

  data[nodeName] = data._hashedResponses[hashKey] = depsPromise
    .then(function (results) {
      var args = results = results.slice(0) //node.silentInputs.length)
