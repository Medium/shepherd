...
function CompiledNode(
  ...
  if (literal) {
    var literalHash = this._graph._valueHasher.getHash(literal)
    var literalPromises = graph.getInternalPromiseMap()
    var literalValues = graph.getInternalValueMap()
...
CompiledNode.prototype.generateHashes = function (nodes, fnOriginNames, idProvider) {
  ...
  if (this.literalPromise) {
    hashBuilder.add(this._graph._valueHasher.getHash(this.literalPromise))
  }

  // compute the function hash, and mix that in.
  var fnHash = this._graph._valueHasher.getHash(this.fn)
  ...
  if (this.errorFn) {
    hashBuilder.add(this._graph._valueHasher.getHash(this.errorFn))
  }
