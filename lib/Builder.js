...
Builder.prototype._getNodeFromDotGraph = function (g, nodes, nodeName, isOutput) {
  // nodes that are passed in as inputs are red, implicitly created nodes are blue
  var color = nodeName.match(/^(req|res|context)(\..*)?$/) ? 'red' : 'blue'

  if (!nodes[nodeName]) {
    // set up the parent
    var parentNode, matches
    if (matches = nodeName.match(/^(.*)\.[^\.]+$/)) parentNode = this._getNodeFromDotGraph(g, nodes, matches[1])
    ...
    if (compiledNode) {
      if (compiledNode.copyOf) return nodes[nodeName] = this._getNodeFromDotGraph(g, nodes, compiledNode.copyOf, isOutput)
      ...
      for (var input in this._compiled.nodes[nodeName].inputs) {
        nodes[input] = this._getNodeFromDotGraph(g, nodes, this._compiled.nodes[nodeName].inputs[input])
...
Builder.prototype.getDotGraph = function (inputNodes) {
  ...
  for (nodeName in this._builds) {
    node = this._getNodeFromDotGraph(g, nodes, nodeName, true)
