...
function CompiledNode(
  ...
  // The local input names for this graph, organized by type.
  // TODO(nick): Change this representation.
  this._argInputs = argInputs
  this._importantInputs = importantInputs
...
CompiledNode.prototype.isEvaluatedInput = function (localName) {
  return this._importantInputs.indexOf(localName) !== -1 || this._argInputs.indexOf(localName) !== -1
}
...
CompiledNode.prototype.getImportantInputGlobalNames = function () {
  ...
  for (var i = 0; i < this._importantInputs.length; i++) {
    var inputName = this.inputs[this._importantInputs[i]]
    var inputRootName = utils.getNodeRootName(inputName)
    result.push(inputRootName)
  }
  return result
}
...
CompiledNode.prototype.getNormalInputGlobalNames = function () {
  ...
  for (var i = 0; i < this._argInputs.length; i++) {
    var inputName = this.inputs[this._argInputs[i]]
    var inputRootName = utils.getNodeRootName(inputName)
    result.push(inputRootName)
  }
  return result
...
CompiledNode.prototype.getAllInputNames = function () {
  return this._importantInputs.concat(this._argInputs)
}
...
CompiledNode.prototype.generateRecursiveInputs = function (nodes) {
  ...
  var shouldSendInputs = !this.isConditional && !this.hasGettersEnabled
  ...
  for (var inputName in this.inputs) {
    var inputNode = nodes[utils.getNodeRootName(this.inputs[inputName])]
    if (inputNode) {
...
Builder.prototype._setImportantRecursively = function () {
  ...
  do {
    ...
    for (key in this._compiled.nodes) {
      ...
      for (var inputKey in node.inputs) {
        if (node.isImportant || node.isImportantInput(inputKey)) {
...
Builder.prototype._addImportantDependenciesForNode = function (nodeName, deps) {
  ...
  for (i = 0; i < deps.length; i++) {
    ...
    for (var inputName in node.inputs) {
      var inputRootName = utils.getNodeRootName(node.inputs[inputName])
      if (inputRootName == dep) {
        found = true
      }
    }
    if (!found) {
      node.addImplicitImportantInput(dep)
    }
  }

  var childDeps = node.getImportantInputGlobalNames()
  var inputNames = node.getNormalInputGlobalNames()
  for (i = 0; i < inputNames.length; i++) {
    var inputNode = this._compiled.nodes[inputNames[i]]
    if (inputNode && !inputNode.isImportant) {
      this._addImportantDependenciesForNode(inputNames[i], childDeps)
    }
  }
}
...
Builder.prototype._getQuietInputValidator = function (nodeName) {
  ...
  return function importantInputValidator(data) {
    ...
    if (err instanceof ConditionalError && node.isImplicitImportantInput(inputRootName)) err = undefined
    return err
  }
}
