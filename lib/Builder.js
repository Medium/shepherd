...
Builder.prototype._generateHashesForNode = function (nodeName) {
  ...
  node.completeHash = crypto.createHash('md5').update(JSON.stringify(node, function (key, val) {
    if (typeof val == 'object' && val != null) {
      var hash = oid.hash(val)
      if (typeof val === 'object' && foundHashes[oid.hash(val)]) return hash
      foundHashes[oid.hash(val)] = true
    }
  ...
  foundHashes = {}
  node.nonSilentHash = crypto.createHash('md5').update(JSON.stringify(node, function (key, val) {
    if (typeof val == 'object' && val != null) {
      var hash = oid.hash(val)
      if (typeof val === 'object' && foundHashes[oid.hash(val)]) return hash
      foundHashes[oid.hash(val)] = true
...
Builder.prototype._run = function (inputData) {
  if (!this._compiled) this.compile(Object.keys(inputData || {}))
...
Builder.prototype._setupChildNodes = function (def, depManager, args, peers) {
  ...
  for (i = 0; i < def.getBuilds().length; i += 1) {
    ...
    for (j = 0; j < toBuild.provides.length; j += 1) {
      ...
      if (provided.nodeName === utils.INPUT_ARG_PREFIX + utils.WILDCARD_PATTERN) {
      ...
      } else if (provided.nodeName.indexOf(utils.INPUT_ARG_PREFIX) === 0) {
        // node takes an argument that was passed into this node
        nodeName = provided.nodeName.substr(utils.INPUT_ARG_PREFIX.length)
        nodeRoot = utils.getNodeRootName(nodeName)

        newRootNode = args[nodeRoot]
        if (!newRootNode) throw new Error("Unable to find node '" + provided.nodeName + "' (passed from '" + def.getName() + "' to '" + toBuild.field + "')")
        if (peers[newRootNode]) newRootNode = peers[newRootNode].newName
        provideMap[field][provided.arg] = utils.swapNodeRoot(provided.nodeName.substr(utils.INPUT_ARG_PREFIX.length), newRootNode)
...
function handleTypeError(enforceTypes, err) {
  if (enforceTypes == 'throw') {
    throw new Error(err)
  } else if (enforceTypes == 'warn') {
    console.warn(err)
    console.trace()
  }
}
...
Builder.prototype._getSuccessHandler = function (nodeName) {
  ...
  var enforceTypes = this._config.enforceTypes
...
Builder.prototype._getCompleteHandler = function (nodeName) {
  ...
  return function (response, data) {
    ...
    if (nextNodes.length === 1) {
      // if there is only a single node to build, build it immediately
      resolvers[nextNodes[0]](data)
