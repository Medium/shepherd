...
CompiledNode.prototype.generateHashes = function (nodes, fnOriginNames, idProvider) {
  ...
  if (this.errorFn) {
    hashBuilder.add(oid.hash(this.errorFn))
  }
  ...
  while (nodesToCheck.length) {
    ...
    if (compiledNode && compiledNode.fn) {
      ...
      for (i = 0; i < allInputNames.length; i += 1) {
        // Check to see if a node was improperly remapped over a runInput
        var inputName = allInputNames[i]
        var remappedInput = compiledNode.inputs[inputName]
        if (runInputsDict[inputName] && remappedInput != inputName) {
          throw new Error('Bad override of input ' + inputName + ' at node ' + node.prefix + '->' + nodeName)
        }

        addNodeFromSource(remappedInput, namePrefix + compiledNode.originalName)
      }
      ...
      if (!runInputsDict[nodeName] && !MAGIC_NODE_NAMES[nodeName]) missingNodes[nodeName] = true
    }
  }
