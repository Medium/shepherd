...
Builder.prototype._compileNode = function (originalNodeName, newNodeName, peers, nodeInputs, nodeModifiers, instance) {
  ...
  if (def._modifiers.length || nodeModifiers.length) {
    this._copyNode(newNodeName, generateModifierNodeName(newNodeName, ++modifierIdx))
  }
  ...
  for (i = 0; i < def._modifiers.length; i += 1) {
    ...
    var currentNodeName = generateModifierNodeName(newNodeName, modifierIdx)
    var nextNodeName = generateModifierNodeName(newNodeName, ++modifierIdx)
