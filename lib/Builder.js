  ...
  if (runInputs) {
    ...
    function addNodeFromSource(nodeName, source) {
      ...
      if (!nodesRequestedBy[nodeNameRoot]) {
        ...
        nodesToCheck.push(nodeNameRoot)
      }
      nodesRequestedBy[nodeNameRoot][source] = true
    }

    // start checking with nodes that we know need to be built
    for (key in this._builds) addNodeFromSource(key, '__BUILDER__')
    for (key in this._silentBuilds) addNodeFromSource(key, '__BUILDER__')

    // loop until we can't loop no more
    while (nodesToCheck.length) {
      nodeName = nodesToCheck.shift()
      ...
      if ((!compiledNode || !compiledNode.fn) && runInputs.indexOf(nodeName) === -1) missingNodes[nodeName] = true
    }
