...
Builder.prototype._compileNode =
  ...
  // create the node
  var isGloballyCacheable = (inputMetadata.length === 0 && !def.isCacheDisabled())
  if (isGloballyCacheable) {
    if (this._compiled.globalNodeCache[originalNodeName]) {
      return this._compiled.globalNodeCache[originalNodeName]
    }
  }

  var node = this._compiled.nodes[newNodeName] = new CompiledNode(
      this._graph, this._config, originalNodeName, newNodeName, inputs,
      inputMetadata, def)

  if (isGloballyCacheable) {
    this._compiled.globalNodeCache[originalNodeName] = node
  }
  return node
}
