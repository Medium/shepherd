...
/**
 * Retrieve a list of all node names to be built importantly for this builder
 *
 * @return {Array.<string>} list of important output node names
 */
Builder.prototype.getImportantOutputs = function () {
  return Object.keys(this._importantBuilds)
}
      ...
      if (!node.outputNodes.length) {
        for (var inputKey in node.inputs) {
          ...
          if (inputNode) {
            ...
            if (inputNode.importantOutputs[key]) delete inputNode.importantOutputs[key]
          }
        }
        delete this._compiled.nodes[key]
        found = true
...
Builder.prototype._generateRecursiveFailureChainForNode = function (nodeName, failureChain, callerNode) {
  ...
  // TODO(nick): We need a better output format. The newName is too inscrutable
  // for normal users.  Our own error-handlers filter this out.
...
Builder.prototype._compileNode = function (originalNodeName, newNodeName, peers, nodeInputs, isConditional) {
  ...
  return this._compiled.nodes[newNodeName] = new CompiledNode(
      ...
      argInputs, importantInputs, def, isConditional)
}
