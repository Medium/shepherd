...
Builder.prototype._setupArgumentsForNode = function (depManager, def, nodeInputs, peers, inputMetadata) {
  ...
  for (var i = 0; i < def.getArgs().length; i += 1) {
    ...
    if (nodeInputs[argAliasRealName]) {
      ...
      this._assertAccessibleInScope(def.getName(), inputRootNode)
      ...
      this._assertAccessibleInScope(def.getName(), argNode.rootName)
...
Builder.prototype._setupChildNodes = function (def, depManager, args, peers) {
  ...
  for (i = 0; i < def.getBuilds().length; i += 1) {
    ...
    this._assertAccessibleInScope(def.getName(), toBuildNode.rootName)
...
/**
 * Throws an error if the named node cannot be a a node within a given scope.
 *
 * @param {string} fromNodeName The node that is initiating the access (by doing a .builds() or a .using())
 * @param {string} toNodeName The node that we are trying to access
 */
Builder.prototype._assertAccessibleInScope = function (fromNodeName, toNodeName) {
  if (utils.isPrivateNode(toNodeName)) {
    console.log('>>>>>> checking to see if ' + toNodeName + ' is accessible from ', fromNodeName)
    var fromDef = this._graph._nodeDefinitions[fromNodeName]
    var toDef = this._graph._nodeDefinitions[toNodeName]

    if (toDef.getScope() !== fromDef.getScope()) {
      throw new Error(
        "Unable to access node '" +
        toNodeName + "' in scope '" + toDef.getScope() + "'" +
        " from node '" + fromNodeName + "' in scope '" + fromDef.getScope() + "'")
    }
  }
}
