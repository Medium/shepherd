...
var NodeDefinition = require('./NodeDefinition') // eslint-disable-line
var NodeInstance = require('./NodeInstance') // eslint-disable-line
...
/**
 * @param {InputMetadata} inputMetadata
 * @return {boolean}
 */
InputMetadata.isImportant = function (inputMetadata) {
  return inputMetadata.important
}
...
function CompiledNode(
  ...
  this.errorFn = def.getErrorFunction()
  this.cacheDisabled = !!def.isCacheDisabled()
  this.hasGettersEnabled = def.hasGettersEnabled()
  this.requiredFields = []
  this.numInputsToResolve = 0
  this.outputNodes = []

  /** @private {boolean} */
  this._lazyDependency = def.isLazyDependency()
  ...
  /** @private {boolean} */
  this._isSingletonScoped = def.isSingletonScoped()

  /** @private {boolean} */
  this._isSingleResult = def.isSingleResult()

  this.type = config.enforceTypes ? originalNodeName.split('-')[0] : null

  this.completeHash = null
  this.nonImportantHash = null

  var literal = this.fn && this.fn._literal
  if (literal) {
    var key
    var literalPromises
    if (utils.isNonObjectLiteral(literal)) {
      key = literal
      literalPromises = graph.getInternalPromiseMap()
    } else {
      key = this._graph._oidProvider.getId(literal)
      literalPromises = graph.getObjectLiteralPromiseMap()
    }

    this.literalValue = literal
    if (!literalPromises[key]) {
      literalPromises[key] = Q.resolve(literal)
    }
    this.literalPromise = literalPromises[key]
  }
}
 ...
 */
CompiledNode.prototype.isLazyDependency = function () {
...
CompiledNode.prototype.getNormalInputGlobalNames = function () {
  var result = [], i = 0
  for (i = 0; i < this._voidInputs.length; i++) {
    result.push(this.localNameToGlobal(this._voidInputs[i]))
  }
  for (i = 0; i < this._argInputs.length; i++) {
...
CompiledNode.prototype.generateHashes = function (nodes, fnOriginNames, idProvider) {
  ...
  if (this.cacheDisabled) {
    // If results caching is disabled, there's no reason to generate a hash.
    this.nonImportantHash = this.completeHash = 'uniq-' + idProvider()
    return
  }
  ...
  if (this.literalPromise) {
    hashBuilder.add(this._graph._oidProvider.getId(this.literalPromise))
  }

  // compute the function hash, and mix that in.
  var fnHash = this._graph._oidProvider.getId(this.fn)
  ...
  if (this.errorFn) {
    hashBuilder.add(this._graph._oidProvider.getId(this.errorFn))
  }
...
/**
 * De-dupes nodes during the initial compilation step.
 *
 * Nodes that are used in many places will get duplicated for each (call,
 * arguments) tuple. To prevent exponential blow-out of common nodes,
 * we try to de-dupe early.
 *
 * This is precarious, because important nodes break when there are diamonds
 * in the dependency graph. So a given Deduper is only valid for a particular
 * important-context.
 *
 * @constructor
 */
function EarlyNodeDeduper() {
  this._singleResultNodes = {}
}
 ...
 */
EarlyNodeDeduper.prototype.add = function (def, node) {
  if (def.isSingleResult()) {
    this._singleResultNodes[node.originalName] = node
  }
}
 ...
 * @param {string} name
 */
EarlyNodeDeduper.prototype.getSharedNode = function (name) {
  return this._singleResultNodes[name]
}
 ...
 *
 * We need two distinct types of hashes to compute equality between nodes.
 * 1) A "complete" hash that includes all inputs, which we use for de-duping nodes.
 * 2) A "non-important" hash that only includes non-important inputs,
 *    which we use for storing intermediate results in GraphResults.
 *
 * The distinction is subtle. Consider the following pseudo-code:
 *
 ...
 * Y(CONFIG)
 *
 * In the data-flow graph, there's a diamond: there are two possible flows through
 * the code, and they both use CONFIG. The traditional way to resolve the
 * cycles in the diamond is with a phi function, which is a pain in the ass to
 * compute. See also:
 * http://en.wikipedia.org/wiki/Static_single_assignment_form
 *
 * Shepherd also has this problem: if CONFIG is used in multiple places in the
 * graph, and some of those places are guarded by thunks and conditionals,
 * then we need a phi function that joins those thunks, and only
 * evaluates CONFIG when appropriate. If there are multiple conditionals,
 * the boolean logic could get arbitraily complex (e.g., only evaluate CONFIG
 * if ((x && y) || z)).
 *
 * But I'm pretty lazy, so instead we just duplicate CONFIG multiple times, once
 * for each possible control-flow path, and use the non-important hash to ensure
 * the N nodes only evaluate once. This removes the diamonds from the graph.
 * The cost is that there are some pathological cases where this can
 * blow up to a 2^N compile-time and memory cost (the caching ensures it does not
 * affect run-time cost). So we may eventually need to give up on this approach
 * and add phi functions.
 *
 * @constructor
 */
...
/**
 * Specify that this Builder should configure a specific node name.
 *
 * @param {string|Object} field a field name to configure or an alias to field name mapping
 * @return {NodeDefinition} a NodeDefinition for the specified field
 */
 ...
 *
 * @param {Object|function(Error, Object)} input Any input to be fed into this
 ...
 *     callback
 * @return {Q.Promise.<Object>} promise which returns a key to value
 *     mapping of the result if successful
 */
Builder.prototype.run = function (input, callback, callbackScope) {
  var data = {}
...
/**
 * Creates a new function that injects graph results into the given function,
 * based on parameter name.
 * @param {Function} fn
 * @return {Function}
 */
Builder.prototype.createInjector = function (fn) {

  var argNames = this.mapOutputKeysToArgs(fn)

  return function (data) {
    var args = []
    for (var i = 0; i < argNames.length; i++) {
      args[i] = data[argNames[i]]
    }
...
Builder.prototype._compileOutputs = function () {
  ...
  this._compilePeers(depManager, inputs, modifiers, true, null)
}
...
Builder.prototype._deduplicateNodes = function () {
  var hashNodes = {}
  var remappedNodes = {}
  var key, node, inputName, remappedNode
...
/**
 * Validate that all inputs of a singleton node are also singletons.
 *
 * @param {CompiledNode} compiledNode
 * @param {Array.<string>} runInputs an array of inputs to the graph
 */
Builder.prototype._validateSingletonRelationships = function (compiledNode, runInputs) {
  if (!compiledNode._isSingleResult) {
    throw new Error('Singleton node "' + compiledNode.originalName + '" may not have arguments')
  }

  var inputNames = compiledNode.getNormalInputGlobalNames()
  for (var i = 0; i < inputNames.length; i += 1) {
    var inputNodeName = inputNames[i]
    var inputNode = this._compiled.nodes[inputNodeName]
    if (!inputNode) {
      if (runInputs.indexOf(inputNodeName) != -1) {
        throw new Error('Singleton node "' + compiledNode.originalName + '"' +
                        ' may not depend on runtime input "' + inputNodeName + '"')
      } else {
        throw new Error('Node "' + compiledNode.originalName + '"' +
                        ' missing input "' + inputNodeName + '"')
      }
    } else if (!inputNode._isSingletonScoped) {
      throw new Error('Singleton node "' + compiledNode.originalName + '"' +
                      ' may not depend on non-singleton input "' + inputNode.originalName + '"')
    }
  }
}
...
Builder.prototype._validateBuilder = function (runInputs) {
  ...
  while (nodesToCheck.length) {
    ...
    if (compiledNode && compiledNode.fn) {
      ...
      // Check singleton relationships.
      if (compiledNode._isSingletonScoped) {
        this._validateSingletonRelationships(compiledNode, runInputs)
      }
      ...
      if (this._config.enforceMatchingParams &&
        ...
        var declaredParams = compiledNode.getArgumentInputNames().map(function (param) {
          ...
          return utils.getNodeInjectorName(fullParamName)
        })
  ...
  for (var key in this._compiled.nodes) {
    ...
    for (var inputKey in node.inputs) {
      ...
      if (node.isEvaluatedInput(inputKey)) {
        ...
        // Lazy dependencies don't need to be resolved.
        if (!inputNode.isLazyDependency()) {
          node.numInputsToResolve++
        }
      } else {
...
Builder.prototype._prepareInputlessLiterals = function () {
  for (var key in this._compiled.nodes) {
    var node = this._compiled.nodes[key]
    if (node.literalPromise && node.numInputsToResolve === 0 && node.outputNodes.length > 0) {
...
Builder.prototype._findStartingNodes = function () {
  for (var key in this._compiled.nodes) {
    var node = this._compiled.nodes[key]
    if (node.numInputsToResolve === 0 && !node.isInput) {
...
Builder.prototype._setupChildNodes = function (def, depManager, args, peers) {
  ...
  for (i = 0; i < def.getBuilds().length; i += 1) {
    ...
    this._checkScope(toBuildNode, def, scope)
    ...
    for (j = 0; j < toBuild.provides.length; j += 1) {
      ...
      var providedName = /** @type {string} */ (provided.nodeName)

      if (utils.isWildcardArgRef(providedName)) {
      ...
      } else if (utils.isArgRef(providedName)) {
        // node takes an argument that was passed into this node
        nodeName = /** @type {string} */ (utils.getArgRef(providedName))
        nodeRoot = utils.getNodeRootName(nodeName)

        newRootNode = args[nodeRoot]
        if (!newRootNode) throw new Error('Unable to find node "' + providedName + '" (passed from "' + def.getName() + '" to "' + toBuild.field.aliasRealName + '")')
        if (peers[newRootNode]) newRootNode = peers[newRootNode].newName
        provideMap[field][provided.arg] = utils.swapNodeRoot(nodeName, newRootNode)

      } else {
        // build a new node (as a peer)
        this._checkScope(provided, def, scope)
        provideMap[field][provided.arg] = providedName
        depManager.addDependency(field, provided.getRootName())
      }
    }
  }
...
/**
 * Checks that the given Node matches the given scope. Throws an error on failure.
 * @param {./utils.NodeInfo|./utils.NodeArgInfo} node
 * @param {NodeDefinition} caller
 * @param {string} scope
 */
Builder.prototype._checkScope = function (node, caller, scope) {
  var rootName = node.getRootName()
  if (rootName && utils.isPrivateNode(rootName)) {
    var buildDef = this._graph._nodeDefinitions[rootName]
    if (buildDef.getScope() !== scope) {
      throw new Error(
          'Unable to access node "' +
          rootName + '" in scope "' + buildDef.getScope() + '"' +
          ' from node "' + caller.getName() + '" in scope "' + scope + '"')
    }
  }
}
 ...
 * @param {?EarlyNodeDeduper} deduper
 * @return {Object} a compiled (data) representation of the node
 */
Builder.prototype._compileNode =
    function (originalNodeName, newNodeName, peers, nodeInputs, deduper) {
  ...
  if (!deduper) {
    deduper = new EarlyNodeDeduper()
  }

  var dupe = deduper.getSharedNode(originalNodeName)
  if (dupe) {
    return dupe
  }
  ...
  // add any remaining nodes as void inputs
  for (key in nodeInputs) {
    ...
    var argNode = peers[nodeRealName]
    inputs[key] = argNode ? argNode.newName : nodeRealName

    var inputNode = utils.getNodeInfoFromInput(this._graph, nodeInputs[key])
    var isImportant = !!inputNode && inputNode.isImportant() || utils.isImportantNode(key)
    inputMetadata.push(new InputMetadata(key).setImportant(isImportant).setArgument(false))
  }
  ...
  // If we have important inputs, then each child will need their own early deduper,
  // because they each have a unique important context.
  var childDeduper = inputMetadata.some(InputMetadata.isImportant) ? null : deduper
  var children = this._compilePeers(depManager, childData.provideMap, childData.modifierMap, false, childDeduper)
  for (key in args) {
  ...
  var compiledNode = this._compiled.nodes[newNodeName] = new CompiledNode(
      this._graph, this._config, originalNodeName, newNodeName, inputs,
      inputMetadata, def)
  deduper.add(def, compiledNode)
  return compiledNode
}
 ...
 */
Builder.prototype._compilePeers = function (depManager, nodeInputs, nodeModifiers, isBuilder, deduper) {
  var i
  var nodeSuffix = isBuilder ? '' : '-peerGroup' + (++this._compiled.nodePrefixIdx)
  var peers = {}
  var compiledAnyNode, compiledCurrentNode, key
  var depsToBuild = {}
  ...
  do {
    ...
    for (key in depsToBuild) {
      ...
      if (compiledCurrentNode) {
        ...
        peers[key] = this._compileNode(nodeName, key + nodeSuffix, peers, utils.clone(nodeInputs[key] || {}), deduper)
      }
    }
 ...
 * @return {Q.Promise.<Object>} a promise with the output of the builder
 */
...
Builder.prototype._getCompleteHandler = function (nodeName) {
  ...
  // If this is a lazy dependency, do nothing. The handler function will do all the work.
  if (node.isLazyDependency()) return utils.nullFunction
  ...
  return function onComplete(data) {
    if (node.isOutput) {
    ...
    for (var i = 0; i < node.outputNodes.length; i++) {
      ...
      if (outputNode.numInputsToResolve === data._numResolvedInputs[outputNodeName]) {
...
/**
 * Create a quiet input validator for a graph node
 *
 * @param {string} nodeName
 * @return {function(Object)|undefined} the quiet input validator which will
 *     return an error if a quiet input has failed
 */
Builder.prototype._getQuietInputValidator = function (nodeName) {
  ...
  return function quietInputValidator(data) {
    var err

    // make sure all quiet inputs worked
    for (var i = 0; i < quietInputNames.length && !err; i++) {
      ...
      if (data._errors[inputRootName]) {
        ...
        try {
          getChildGetter(inputName)(data.getResult(inputRootName, node))
        } catch (e) {
          err = e
        }
      }
...
/**
 * Create an argument input validator for a graph node
 *
 * @param {string} nodeName
 * @return {function(Object, Array)|undefined} the argument input validator
 *     which will return an error if an input has failed. Modifies the array of
 *     arguments in place for consumption by the resolvers
 */
Builder.prototype._getArgumentInputValidator = function (nodeName) {
  ...
  return function argumentInputValidator(data, args) {
    var err, val
 ...
 *
 * @param {string} nodeName
 * @param {function(CompiledNode, Array.<NodeResponseGetter>) : ?} resolveNodeFn
 ...
 * @return {Q.Promise.<Object>} The result of the resolved node
 */
Builder.prototype._resolveLiteral = function (node) {
 ...
 * @param {CompiledNode} node The node to resolve
 * @param {Array.<NodeResponseGetter|Object>} args The argument inputs
 * @return {Q.Promise.<Object>|Object} The result of the resolved node
 */
