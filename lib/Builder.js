...
Builder.prototype._generateRecursiveInputsForNode = function (nodeName) {
  ...
  for (i = 0; i < node.silentInputs.length; i++) {
    ...
    if (node.allInputs[childNodeName]) {
      ...
      var found = false
      var j
      for (var j = i; j < node.silentInputs.length && !found; j++) {
        if (node.silentInputs[j] === childNodeName) found = true
      }

      for (var j = 0; j < node.argInputs.length && !found; j++) {
        if (node.argInputs[j] === childNodeName) found = true
      }
    } else {
...
Builder.prototype._getErrorHandler = function (nodeName) {
  ...
  return function (e, data) {
    if (!(e instanceof ConditionalError) && !e.graphInfo) {
      var depNodes = []
