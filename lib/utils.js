...
/**
 * Class for providing hash keys for literal values and objects.
 * @constructor
 */
function ValueHasher() {
  var initialOid = 5 // Start after the predefined values

  /**
   * Unique object id provider for objects and functions
   * @type {Oid}
   */
  this._oidProvider = new Oid(initialOid)
}

ValueHasher.prototype.UNDEF_VAL = 1
ValueHasher.prototype.NULL_VAL = 2
ValueHasher.prototype.TRUE_VAL = 3
ValueHasher.prototype.FALSE_VAL = 4

/**
 * Get an integer hash key for a value (a literal or object)
 *
 * @param {Object} obj
 * @return {number} the identifier
 */
ValueHasher.prototype.getHash = function(val) {
  if (typeof val === 'undefined') {
    return ValueHasher.prototype.UNDEF_VAL
  }
  if (val === null) {
    return ValueHasher.prototype.NULL_VAL
  }
  if (typeof val === 'boolean') {
    return val ? ValueHasher.prototype.TRUE_VAL : ValueHasher.prototype.FALSE_VAL
  }
  ...
  // objects and functions get a unique id
  return this._oidProvider.getId(val)
}
 ...
 * @constructor
 */
function Oid(initialOid) {
  /**
   * Map of object to its identifier. We use WeakMap to avoid keeping references to the objects
 ...
 * @return {number} the identifier
 */
Oid.prototype.getId = function (obj) {
  if (this._objectToId.has(obj)) {
    return this._objectToId.get(obj)
  }
  this._idCounter++
  this._objectToId.set(obj, this._idCounter)
