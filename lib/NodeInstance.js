...
NodeInstance.prototype.builds = function (name) {
  ...
  if (!utils.isSilentNode(nodeInfo.rawName) && !utils.isOptionalNode(nodeInfo.rawName)) {
    // if the node has been explicitly aliased, map to the alias, otherwise use the real name
    var outputName = typeof name == 'object' ? nodeInfo.alias : nodeInfo.fullName
    var outputNodeInfo = utils.getNodeInfoFromBuild(this.getGraph(), outputName)
    if (outputNodeInfo.fullName == outputName && this._outputFieldNames.indexOf(outputNodeInfo.fullName) === -1) {
...
/**
 * Proxy function which can be used to chain setDebugger() requests back
 * to the builder
 */
NodeInstance.prototype.setDebugger = function () {
  return this._builder.setDebugger.apply(this._builder, arguments)
}
