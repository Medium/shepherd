...
NodeInstance.prototype.using = function (var_args) {
  for (var i = 0; i < arguments.length; i++) {
    ...
    var nodeInfo = utils.getNodeInfoFromInput(this._builder.getGraph(), arg)
    if (utils.isPrivateNode(nodeInfo.nodeName)) throw new Error('Private nodes may not be accessed from .using() in builders')
    ...
    if (Array.isArray(nodeNames)) {
      ...
      var wrapperNodeName = this._builder.getGraph().addAnonymous(nodeInfo.arg, this._builder.getGraph().argsToArray, wrapperArgs)
