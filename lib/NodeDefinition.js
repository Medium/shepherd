...
NodeDefinition.prototype._addCondition = function (field, isUnless) {
  ...
  var isArg = nodeInfo.fullName.indexOf(utils.INPUT_ARG_PREFIX) === 0
  ...
  if (!isArg && !this._getNode(nodeInfo.alias)) {
    ...
    if (this._conditionalState.candidates.length) {
      response.using(utils.OUTPUT_PREFIX_SILENT + this._getFutureWhenName(false, -1))
    }
  }
...
NodeDefinition.prototype.end = function () {
  if (this._conditionalState.nextCandidate) {
    ...
    var elseBuild = this._buildsWithWhen(elseObj, true)
      .using(utils.OUTPUT_PREFIX_SILENT + this._getFutureWhenName(false, -1))
  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (Array.isArray(nodeNames)) {
      if (!nodeNames.length) {
        ...
        var newBuilds = this.builds(utils.OUTPUT_PREFIX_SILENT + wrapperNodeName)
    ...
    } else if (typeof nodeNames == 'string' && nodeInfo.nodeName.indexOf(utils.INPUT_ARG_PREFIX) === 0) {
      // check that an existing arg is referenced
      var inputArg = nodeInfo.nodeName.substr(utils.INPUT_ARG_PREFIX.length).split('.')[0]
      if (inputArg != utils.WILDCARD_PATTERN && this._inputArgs.indexOf(inputArg) === -1) {
...
NodeDefinition.prototype.modifiers = function (var_args) {
  ...
  for (var i = 0; i < arguments.length; i += 1) {
    if (this._builds.length) {
      ...
      // rename the node
      var newNodeInfo = {}
      newNodeInfo[utils.OUTPUT_PREFIX_SILENT + nodeName] = nodeInfo.fullName
...
NodeDefinition.prototype._buildsWithWhen = function (field, isWhen) {
  var whenNode = null
  var nodeInfo = utils.getNodeInfoFromBuild(this._graph, field)
  if (this._getArgIdx(nodeInfo.alias) !== -1) {
  ...
  if (this.isConditional()) {
    ...
    if (!utils.isSilentNode(nodeInfo.alias)) {
      ...
      field = {}
      field[utils.OUTPUT_PREFIX_SILENT + nodeInfo.alias] = nodeInfo.fullName
  ...
  if (whenNode) {
    ...
    this.using(utils.OUTPUT_PREFIX_SILENT + whenNode)
  }
  return this
}
