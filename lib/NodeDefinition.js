 ...
 */
NodeDefinition.prototype.ensureComplete = function () {
...
NodeDefinition.prototype._getArgIdx = function (nodeInfo) {
  for (var i = 0; i < this._args.length; i++) {
    var argInfo = utils.getNodeInfoFromBuild(this._graph, this._args[i])
    if (nodeInfo.isSameAlias(argInfo)) return i
  }
...
NodeDefinition.prototype._buildsWithWhen = function (field, isWhen) {
  ...
  if (this.isConditional()) {
    ...
    if (!nodeInfo.isSilent()) {
      // silent nodes still need to depend on the conditional
      field = {}
      field[utils.OUTPUT_PREFIX_SILENT + nodeInfo.aliasRealName] = nodeInfo.fullName
