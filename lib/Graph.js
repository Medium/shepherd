...
function Graph() {
  ...
  this.type('builderOutput', Object)
...
Graph.prototype.enforceTypes = function (level) {
  return this.setConfig('enforceTypes', level || 'throw')
}
...
Graph.prototype.enforceBuilderNames = function (level) {
  return this.setConfig('enforceBuilderNames', level || 'throw')
}
...
Graph.prototype.addBuilderHandler = function (builder, fn) {
  var name = 'builderOutput-' + builder.getName().replace(/[^\w\d\-]+/g, '') + '_' + (++this._state.anonymousFnCounter)
...
Graph.prototype.type = function (key, val) {
  if (this._config.forceClone) return this.clone().type(key, val)
...
Graph.prototype._provide = function (nodeName, key, val) {
  if (key === utils.OUTPUT_PREFIX_SILENT) {
    val = Array.isArray(val) ? val : [val]
    for (var i = 0; i < val.length; i += 1) {
      val[i] = utils.OUTPUT_PREFIX_SILENT + val[i]
...
Graph.prototype._notifyInvalidBuilderName = function (msg) {
  if (this._config.enforceBuilderNames == 'warn') {
    console.warn(msg)
    console.trace()
  } else if (this._config.enforceBuilderNames == 'throw') {
    throw new Error(msg)
  }
}
