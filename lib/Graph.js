...
function Graph() {
  ...
  this._literalFns = {}
  this._objectLiteralFns = {}

  /** @private {Object.<Q.Promise>} a map of literal values to promises */
  this._literalPromises = {}

  /** @private {Object.<Q.Promise>} a map of object literal oids to promises */
  this._objectLiteralPromises = {}

  /** @private {utils.Oid} provides object ids unique to this graph */
  this._oidProvider = new utils.Oid()
 ...
 * @return {Object.<Q.Promise>}
 */
Graph.prototype.getObjectLiteralPromiseMap = function () {
  return this._objectLiteralPromises
}
...
Graph.prototype.literal = function (val) {
  ...
  var key
  var fnMap
  if (utils.isNonObjectLiteral(val)) {
    key = val
    fnMap = this._literalFns
  } else {
    key = this._oidProvider.getId(val)
    fnMap = this._objectLiteralFns
  }

  if (fnMap[key]) return fnMap[key]
  return fnMap[key] = makeLiteralHandler(val)
}
