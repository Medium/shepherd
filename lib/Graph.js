...
function Graph() {
  ...
  /** @private {utils.ValueHasher} provides hashes of values and object ids unique to this graph */
  this._valueHasher = new utils.ValueHasher()
...
Graph.prototype.literal = function (val) {
  ...
  // This isn't guaranteed to be unique. Specifically, a number or string literal could collide
  // with the hash of an object literal. Consider maintaining a separate map for number and string
  // literals where the key is the literal.
  var hash = this._valueHasher.getHash(val)
