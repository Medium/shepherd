 ...
 * @return {Graph} the current graph instance
 */
Graph.prototype.enforceTwoPartNames = function () {
  return this.setConfig('enforceTwoPartNames', true)
}
...
Graph.prototype.add = function (name, fn, deps) {
  ...
  if (this._config.enforceTwoPartNames) {
    ...
    if (idx === -1) throw new Error("Node '" + name + "' must be hyphen-delimited")
    else if (idx === 0) throw new Error("Node '" + name + "' must not start with -")
    else if (idx === name.length - 1) throw new Error("Node '" + name + "' must not end with -")

    idx = name.indexOf('-', idx + 1)
    if (idx !== -1) throw new Error("Node '" + name + "' may only have a single -")
  }
...
Graph.prototype.provideTo = function (nodeName, provides) {
  if (this._config.forceClone) return this.clone().provideTo(nodeName, provides)
...
/**
 * Add a modifier node to another node in this Graph
 *
 * @param {string} nodeName the node to modify
 * @param {string|Object} modifierData the modifier's node information
 */
Graph.prototype._addModifier = function (nodeName, modifierData) {
  var modifierInfo = utils.getNodeInfoFromModifier(this, modifierData, nodeName)
  ...
  } while (this._nodeDefinitions[newName])

  this._nodeDefinitions[newName] = this._nodeDefinitions[nodeName]
  this._nodeDefinitions[newName].setName(newName)
  var inputs = this._nodeDefinitions[newName].getInputNames()
  delete this._nodeDefinitions[nodeName]

  var newNodeUsing = {}
  newNodeUsing[nodeShortName] = newName

  var proxyNode = this.add(nodeName, this.subgraph)
  proxyNode.args.apply(proxyNode, inputs)

  proxyNode
    .builds(newName)
      .using('args.*')
  proxyNode
    .builds(modifierNodeName)
      .using(newNodeUsing)
}
